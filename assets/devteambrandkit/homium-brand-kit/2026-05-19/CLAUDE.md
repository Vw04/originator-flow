...
_Last updated: 2026-05-19. Heading typeface migrated to Source Serif 4 (Google Fonts); see Typography → "Source Serif 4 (headings and prose only)."_

---

## How to use this document

1. **Place `CLAUDE.md` at the root of your project.** Claude Code automatically reads any `CLAUDE.md` it finds in the working directory and uses it as its instruction set. Dropping this file in your repo's root makes the design system Claude's source of truth on every task.
2. **Use `index.html` as the implementation reference.** It's the live prototype every pattern in this document was built against. Open it in a browser to inspect components; grep it for usage examples (e.g., `class="DetailHeaderCard"` to see real usage); copy patterns directly into your codebase.
3. **Keep this document evergreen.** When a new pattern is introduced — and approved (see Working Agreement below) — document it here in the same pass. Treat doc-debt the same as code-debt: don't ship a new pattern without its doc.

---

## Working Agreement (for Claude Code — and any contributor)

This section is the behavioral contract Claude Code follows when working on Homium. Read it before every task. It applies whether you're updating the prototype itself or applying these patterns to a separate Homium codebase.

### The default — always

- **Describe before tweaking.** Before touching any file, state clearly what you're about to change and why. Wait for confirmation. For small edits, a one-line preview is enough; for larger changes, present a brief plan with file paths and rationale. The user always sees what's coming before it lands.
- **Reuse over invent.** Use existing tokens, components, and patterns documented here. Never invent a new color value, new spacing unit, new pill shape, new button variant, or new layout convention without explicit approval.
- **Surface options.** When a task could go several ways, present 1–3 approaches with tradeoffs and let the user choose. Don't pick silently. This is especially important for design questions — color semantics, button hierarchy, table behavior — where reasonable people disagree.
- **Files are the source of truth.** Read the current state of the file before editing. Don't work from memory of past conversations.

### Stop and ask before:

- Adding a **new library component** — Modal variants, new card types, new button modifiers, new chart types, new form-field variants. These belong in the design system; one-off page-local code is the wrong default.
- Adding or modifying **design tokens** — colors, spacing scale entries, type scale entries, radius values, shadows.
- Introducing a **new pattern** — new table column type, new table behavior (sorting, filtering, expansion, virtualization), new sticky-bottom variant, new sub-routing model, new dirty-state mechanism, new keyboard shortcut.
- **Breaking changes** to existing primitives — renaming a class, changing a shape vocabulary entry, retiring a component, changing a CSS variable's meaning.
- **Out-of-scope edits** — if you spot consistency drift or a bug while doing requested work, flag it as separate follow-up work. Don't fix it silently in the same PR.
...
- Restate your change plan before making the edit. That's confirming intent, not asking permission.

### When the user asks for something this document doesn't cover

Don't improvise. Reply with:

1. **What pattern they're asking for** (your interpretation).
2. **What already exists nearby** that might solve it.
3. **2–3 options** if you'd need to add something new, with tradeoffs (cost, reusability, scope).
4. **Your recommendation** if you have a clear one, but ask before implementing.

This is how the design system stays coherent over time — every new pattern is a deliberate decision, not an accident.
...
| Looking for… | Section |
|---|---|
| Colors, tokens, dark mode | [Design Tokens](#design-tokens) |
| Type scale, font choices, numeric content | [Typography](#typography) |
| Spacing scale, radii, shape vocabulary | [Spacing, radii, shape](#spacing) (within Design Tokens) |
| Button variants and when to use which | [Buttons & Action Hierarchy](#buttons--action-hierarchy) |
| Tables, columns, cells, footers, patterns | [Tables](#tables) |
| Cards (table card, detail card, KPI card, chart card) | [Cards](#cards) |
| Forms — fields, sections, save bars, dirty modal | [Forms](#forms) |
| Page navigation, back links, top-nav structure | [Navigation patterns](#navigation-patterns) |
| Status pills, role pills, tags, badges | [Pills & Tags](#pills--tags) |
| Modals and confirm dialogs | [Modal (library)](#modal-library) |
| Permissions matrix, market grid, dot semantics | [PermissionsMatrix](#permissionsmatrix), [MarketEnablementGrid](#marketenablementgrid) |
| Layouts (DetailPage, PageHeader, surface hierarchy) | [Layout](#layout) |
| Toast (transient feedback) | [Toast](#toast) |
| Dev widget (bottom-right) | [Dev Widget](#dev-widget) |
| Sticky bars (`--sticky-bottom-offset`) | [Forms → Save bar](#save-bar-sticky-bottom) |

---

## Golden Rules (carried from the legacy preamble)
...
5. **Index pages separately.** Each page/screen is its own file when building production. The prototype keeps them in a single `index.html` for prototype-speed reasons; production should split. Include a small dev widget (bottom-right corner, collapsible) that shows current user role + current page name in dev mode only.
...
All numerical content uses Inter with `font-feature-settings: 'tnum' 1, 'lnum' 1`. Source Serif 4 is reserved for prose headings and large display numbers (KPI values, entity counts) — NEVER appears in dense chart labels or tabular financial data. JetBrains Mono is used only for technical identifiers (Loan IDs, MIN numbers, hashes).
...
| Value (metric) | `ChartTooltip__value` | Source Serif 4 | 18px | 400 | `text-heading` (#00334A) |
...
|---------|------|------|-------|
| Bar value labels ("0.8d", "3.4d") | Source Serif 4 Regular | 13px | #2D3E4A (slate) |
| Count numbers in funnel ("2", "8") | Source Serif 4 Regular | 16px | #2D3E4A (slate) |
...
### Source Serif 4 (headings and prose only)
Open-source modern serif by Frank Grießhammer (Adobe), distributed via Google Fonts. Used for every title / heading surface in the UI — page titles, section titles, card titles, entity-detail headers, KPI display numbers. NEVER used for numbers-as-data inside dense charts/tables (those stay Inter for tabular alignment).

- Loaded from Google Fonts: `family=Source+Serif+4:opsz,wght@8..60,400`.
- Token: `--h-font-heading: 'Source Serif 4', Georgia, 'Times New Roman', serif;` — Georgia stays in the fallback stack for the brief load window and as a graceful degrade.
...
- Feature settings: `font-feature-settings: 'lnum' 1, 'kern' 1; font-variant-numeric: lining-nums;` — Source Serif 4 ships lining figures by default, but `lnum` is set explicitly so the Georgia / Times fallbacks render lining figures too during font load. Applied globally via a selector list covering `.PageHeader__title`, `.SectionTitle`, `.KPICard__value`, `.Activity__title`, `.PropertyHeader__address`, `.PlaceholderPage__title`, `.Drawer__title`, `.Card__title`, `.StepCard__title`, `.ProcessCard__title`, `.CollapsibleCard__title`, and `h1..h6`.

#### Migration note (2026-05-19)
Replaces Georgia, the platform's previous heading serif. Source Serif 4 has proper lining figures and modern OpenType, so the old `HeadingNumFix` @font-face workaround (which mapped digits U+0030–0039 to a locally-installed sans to dodge Georgia's old-style figures) has been removed. KPI values and entity counts now render in the heading serif directly — no digit-substitution trick required.
...
|------|------|--------|------|-------|
| Page title | **Source Serif 4** | 400 (NEVER bold) | 28–32px | `--h-text-heading` |
| Section titles | **Source Serif 4** | 400 | 20px | `--h-text-heading` |
| Address titles (property headers) | **Source Serif 4** | 400 | 20–24px | `--h-text-heading` |
| KPI numbers | **Source Serif 4** | 400 | 36px | `--h-text-heading`, or `--h-alert-text` for alert |
...
- **`HeadingNumFix` @font-face workaround.** Removed when we moved to Source Serif 4 — Source Serif 4 has lining figures natively, so the digit-swap trick is no longer needed. If you see references in older code or comments, they are stale.
- **Inter with slashed zero** (`'zero' 1` or `font-variant-numeric: slashed-zero`). Plain-circle zero only.

### Why these specific choices
- **Source Serif 4 over Georgia:** Source Serif 4 is a modern open-source serif (Adobe / Frank Grießhammer) with proper lining figures, optical sizing, and a richer weight range. Georgia served us well as a zero-load system fallback, but lacked native lining figures (we had to swap digits with an `@font-face` trick) and reads dated next to current product serifs. Georgia remains in the fallback stack so the brief load window and offline-degrade case still ship a credible serif.
...
|---------|------|--------|------|
| Logo (text fallback) | Source Serif 4 | 400 | 22px |
...
**Row 1 — Title:** `.SectionTitle` (Source Serif 4 400, 20px, teal-900) on its own row.
...
├── .PlaceholderPage__title — Source Serif 4 400, 24px, teal-900
...
- ❌ Never bold Source Serif 4 — always font-weight 400 (Regular)
...
- ❌ Never use fonts other than Inter or Source Serif 4 in the nav — no system fonts, Helvetica, or Roboto
...
- **Title in card** — `.SectionTitle` (Source Serif 4 20px) inside the card header. Optional `.SectionSubtitle` (Inter 13px secondary) below.
  - When the card IS the page (no separate `.PageHeader` wrapper above it — e.g., Setup → Companies / Branches / Users list pages), use `.SectionTitle.SectionTitle--page` (Source Serif 4 28px) so the page H1 outranks card section titles in the type scale.
  - **Type scale rule:** page H1 = 28px (`.PageHeader__title` or `.SectionTitle--page`); card section title = 20px (`.SectionTitle`); detail-entity title = 28px Source Serif 4 (`.DetailHeaderCard__title`). Never use 20px for a page-level title.
...
- **Title** — `.DetailHeaderCard__title` (Source Serif 4 28px, font-weight 400)
