...
const DataPlatformView = {
  ...
  /* ================================================================
     INVESTOR DASHBOARD — prospect-style layout + pool analytics
  ================================================================ */
  _renderInvestorDashboard() {
    const data = State.getProspectData();
    if (!data) return '<div class="card">No prospect data available.</div>';

    const filter = this._dashProgramFilter || 'all';
      ...
      <div class="prospect-hero">
        <div class="prospect-hero-inner">
          <div class="prospect-hero-text">
            <span class="prospect-badge" style="background:rgba(255,255,255,0.18)">Investor Dashboard</span>
          ...
          <div class="prospect-hero-stats">
            <div class="prospect-hero-stat">
              <div class="prospect-hero-stat-value">${data.platformKPIs.programsActive}</div>
    ...
    const tabsHtml = tabs.map(t =>
      `<button class="dash-prog-tab ${filter === t.key ? 'active' : ''}"
              onclick="DataPlatformView._setDashProgramFilter('${t.key}')">${t.label}</button>`
    ).join('');

    const programCards = filtered.map(p => `
    ...
    const programSection = `
      <div class="prospect-section">
        ...
        <div class="prospect-programs-grid">${programCards}</div>
      </div>`;

    /* ── Pool Summary (map + charts) ── */
    const poolSection = this._renderPoolSummary();
          ...
          <div class="prospect-comparison">
            <div class="prospect-comparison-col prospect-comparison-before">
              ...
              <div class="prospect-comp-items">
                <div class="prospect-comp-row"><span>Income</span><span>$${bp.income.toLocaleString()}</span></div>
                <div class="prospect-comp-row"><span>Home Price</span><span>$${bp.homePrice.toLocaleString()}</span></div>
                <div class="prospect-comp-row"><span>Monthly PITI</span><span class="prospect-comp-bold">$${bp.beforeHomium.monthlyPITI.toLocaleString()}</span></div>
                <div class="prospect-comp-row"><span>Max Affordable</span><span>$${bp.beforeHomium.maxAffordable.toLocaleString()}</span></div>
              </div>
            ...
            <div class="prospect-comparison-col prospect-comparison-after">
              ...
              <div class="prospect-comp-items">
                <div class="prospect-comp-row"><span>Monthly PITI</span><span class="prospect-comp-bold prospect-comp-green">$${bp.withHomium.monthlyPITI.toLocaleString()}</span></div>
                <div class="prospect-comp-row"><span>Monthly Savings</span><span class="prospect-comp-green">$${bp.withHomium.monthlySavings.toLocaleString()}</span></div>
              </div>
    ...
    const lmHeaderCells = filtered.map(p => `<th style="text-align:right">${p.name}</th>`).join('');
    const lmRows = loanMetrics.map(m => {
    ...
    const loanMetricsSection = `
      <div class="prospect-section">
        <h2 class="prospect-section-title">Key Impact Loan Data</h2>
        <div class="card">
          <div class="table-container" style="border:none;box-shadow:none">
            <table>
              <thead><tr><th>Metric</th>${lmHeaderCells}</tr></thead>
              <tbody>${lmRows}</tbody>
            </table>
          </div>
        </div>
    ...
    const storiesSection = `
      <div class="prospect-section">
        ...
        <div class="prospect-stories-grid">${storiesHtml}</div>
      </div>`;
  ...
  _appDocumentsTab(loan) {
    ...
    return `
      <div class="ud-content-section">
        <div class="ud-dropzone" id="app-dropzone"
             ondragover="event.preventDefault();this.classList.add('dragover')"
             ondragleave="this.classList.remove('dragover')"
             ondrop="event.preventDefault();this.classList.remove('dragover');DataPlatformView._handleFileDrop(event)"
          ...
          <input type="file" id="app-file-input" multiple style="display:none" onchange="DataPlatformView._handleFileSelect(event)">
        </div>
      </div>
      <div class="ud-content-section">
  ...
  _handleFileDrop(event) {
    ...
    if (files?.length) this._showUploadToast(files);
  },
